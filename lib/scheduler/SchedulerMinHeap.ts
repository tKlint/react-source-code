 ...
 * @param {*} node 任务
 */
export function push(heap: Node[], node: Node) {
  // 获取任务队列的长度
 ...
 * @param {*} i 任务队列的长度
 */
function siftUp(heap: Node[], node: Node, i: number) {
 ...
 */
function siftDown(heap: Node[], node: Node, i: number) {
  // 记录任务的下标，也是从 0 开始的
  let index = i;
  // 当前任务队列的长度
